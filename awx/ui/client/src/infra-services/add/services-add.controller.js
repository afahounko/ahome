        ...
        $scope.formSave = function() {
            ...
            Rest.post(data)
                .then(({data}) => {
                    ...
                    if (base === 'ipam_services') {
                        $rootScope.flashMessage = i18n._('New Service successfully created!');
                        $rootScope.$broadcast("EditIndicatorChange", "Service", data.id);
                        
                         $state.go('infraServicesList', null, { reload: true });
                    } else {
                        ReturnToCaller(1);
                    }
                })
                .catch(({data, status}) => {
                    ProcessErrors($scope, data, status, form, { hdr: i18n._('Error!'), msg: i18n._('Failed to add new Service. POST returned status: ') + status });
                });
        };

        $scope.formCancel = function() {
            $state.go('infraServicesList');
        };
    }
];
