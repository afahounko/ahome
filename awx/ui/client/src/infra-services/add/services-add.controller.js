...
export default ['$window', '$scope', '$rootScope', '$stateParams', 'ServiceForm', 'GenerateForm', 'Rest','ParseTypeChange',
    ...
    function($window, $scope, $rootScope, $stateParams, ServiceForm, GenerateForm, Rest, ParseTypeChange, Alert,
	...
	$state, $location, i18n, ParseVariableString, $q, initSelect, cloudProcess, SetActiveWizard, GetOptsValues) {
		...
		$scope.WizardClick = function (clickID) {
			if (clickID == 1) {
				...
				if((form.steps && $scope.tabId == form.steps) || (!form.steps && $scope.tabId == 3)){
					var data = GetOptsValues($scope, form, 'services', fk_type);
        ...
        // Save
        function deleteService(id, _callback){
        	DeleteSubJobTemplate(defaultUrl, id, '', 0);
        	_callback();
        }
        $scope.formSave = function() {
        	var data_item = processNewData(form.fields);
        	SaveInfraItem(defaultUrl, form, data_item, 'infraServicesList');
        };
