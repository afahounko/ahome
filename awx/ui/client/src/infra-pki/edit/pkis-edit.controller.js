...
export default ['$window', '$scope', '$rootScope', '$stateParams', '$timeout', 'PkiForm', 'GenerateForm', 'Rest','ParseTypeChange',
    ...
    function($window, $scope, $rootScope, $stateParams, $timeout, PkiForm, GenerateForm, Rest, ParseTypeChange, Alert,
	...
	$state, $location, i18n, ParseVariableString, initSelect, SetActiveWizard, GetOptsValues) {
        ...
        var master = {}, boxes, box, variable, 
            id = $stateParams.pki_id,
        	fk_type = $window.localStorage.getItem('form_id'),
            form = PkiForm[fk_type],
            defaultUrl = GetBasePath('ipam_pkis') + id;
        ...
        function init() {
            ...
            .catch(({data, status}) => {
                ProcessErrors($scope, data, status, null, {
                    hdr: i18n._('Error!'),
                    msg: i18n.sprintf(i18n._('Failed to retrieve Pki: %s. GET status: '), $stateParams.id) + status
                });
            });
		...
		$scope.WizardClick = function (clickID) {
			if (clickID == 1) {
				...
				if((form.steps && $scope.tabId == form.steps) || (!form.steps && $scope.tabId == 3)){
					var data = GetOptsValues($scope, form, 'pkis', fk_type, prev_opts);
