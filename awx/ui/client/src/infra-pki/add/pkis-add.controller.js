        ...
        $scope.formSave = function() {
            ...
            Rest.post(data)
                .then(({data}) => {
                    ...
                    if (base === 'ipam_pkis') {
                        $rootScope.flashMessage = i18n._('New Pki successfully created!');
                        $rootScope.$broadcast("EditIndicatorChange", "Pki", data.id);
                        
                         $state.go('infraPkiList', null, { reload: true });
                    } else {
                        ReturnToCaller(1);
                    }
                })
                .catch(({data, status}) => {
                    ProcessErrors($scope, data, status, form, { hdr: i18n._('Error!'), msg: i18n._('Failed to add new Pki. POST returned status: ') + status });
                });
        };

        $scope.formCancel = function() {
            $state.go('infraPkiList');
        };
    }
];
