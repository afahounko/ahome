...
export default ['$window', '$scope', '$rootScope', 'StorageForm', 'GenerateForm', 'Rest','ParseTypeChange',
    'Alert', 'ProcessErrors', 'ReturnToCaller', 'GetBasePath',
    'Wait', 'CreateSelect2', '$state', '$location', 'i18n','ParseVariableString',
    function($window, $scope, $rootScope, StorageForm, GenerateForm, Rest, ParseTypeChange, Alert,
        ...
        $scope.formSave = function() {
            ...
            Rest.post(data)
                .then(({data}) => {
                    ...
                    if (base === 'ipam_storages') {
                        $rootScope.flashMessage = i18n._('New Storage successfully created!');
                        $rootScope.$broadcast("EditIndicatorChange", "Storage", data.id);
                        
                         $state.go('infraStoragesList', null, { reload: true });
                    } else {
                        ReturnToCaller(1);
                    }
                })
                .catch(({data, status}) => {
                    ProcessErrors($scope, data, status, form, { hdr: i18n._('Error!'), msg: i18n._('Failed to add new Storage. POST returned status: ') + status });
                });
        };

        $scope.formCancel = function() {
            $state.go('infraStoragesList');
        };
    }
];
