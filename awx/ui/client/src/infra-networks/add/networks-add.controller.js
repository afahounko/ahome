        ...
        $scope.formSave = function() {
            ...
            Rest.post(data)
                .then(({data}) => {
                    ...
                    if (base === 'ipam_networks') {
                        $rootScope.flashMessage = i18n._('New Network successfully created!');
                        $rootScope.$broadcast("EditIndicatorChange", "Network", data.id);
                        
                         $state.go('infraNetworksList', null, { reload: true });
                    } else {
                        ReturnToCaller(1);
                    }
                })
                .catch(({data, status}) => {
                    ProcessErrors($scope, data, status, form, { hdr: i18n._('Error!'), msg: i18n._('Failed to add new Network. POST returned status: ') + status });
                });
        };

        $scope.formCancel = function() {
            $state.go('infraNetworksList');
        };
    }
];
