...
export default ['$window', '$scope', '$rootScope', '$stateParams', '$timeout', 'MonitoringForm', 'GenerateForm', 'Rest','ParseTypeChange',
    ...
    function($window, $scope, $rootScope, $stateParams, $timeout, MonitoringForm, GenerateForm, Rest, ParseTypeChange, Alert,
	...
	$state, $location, i18n, ParseVariableString, initSelect, SetActiveWizard, GetOptsValues) {
        ...
        var master = {}, boxes, box, variable, 
            id = $stateParams.monitoring_id,
        	fk_type = $window.localStorage.getItem('form_id'),
            form = MonitoringForm[fk_type],
            defaultUrl = GetBasePath('ipam_monitorings') + id;
        ...
        function init() {
            ...
            .catch(({data, status}) => {
                ProcessErrors($scope, data, status, null, {
                    hdr: i18n._('Error!'),
                    msg: i18n.sprintf(i18n._('Failed to retrieve Monitoring: %s. GET status: '), $stateParams.id) + status
                });
            });
		...
		$scope.WizardClick = function (clickID) {
			if (clickID == 1) {
				...
				if((form.steps && $scope.tabId == form.steps) || (!form.steps && $scope.tabId == 3)){
					var data = GetOptsValues($scope, form, 'monitorings', fk_type, prev_opts);
