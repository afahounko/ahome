        ...
        $scope.formSave = function() {
            ...
            Rest.post(data)
                .then(({data}) => {
                    ...
                    if (base === 'ipam_monitorings') {
                        $rootScope.flashMessage = i18n._('New Monitoring successfully created!');
                        $rootScope.$broadcast("EditIndicatorChange", "Monitoring", data.id);
                        
                         $state.go('infraMonitoringsList', null, { reload: true });
                    } else {
                        ReturnToCaller(1);
                    }
                })
                .catch(({data, status}) => {
                    ProcessErrors($scope, data, status, form, { hdr: i18n._('Error!'), msg: i18n._('Failed to add new Monitoring. POST returned status: ') + status });
                });
        };

        $scope.formCancel = function() {
            $state.go('infraMonitoringsList');
        };
    }
];
