...
export default ['$window', '$scope', '$rootScope', '$stateParams', '$timeout', 'BackupForm', 'GenerateForm', 'Rest','ParseTypeChange',
    ...
    function($window, $scope, $rootScope, $stateParams, $timeout, BackupForm, GenerateForm, Rest, ParseTypeChange, Alert,
	...
	$state, $location, i18n, ParseVariableString, initSelect, SetActiveWizard, GetOptsValues) {
        ...
        function init() {
            ...
            .catch(({data, status}) => {
                ProcessErrors($scope, data, status, null, {
                    hdr: i18n._('Error!'),
                    msg: i18n.sprintf(i18n._('Failed to retrieve Backup: %s. GET status: '), $stateParams.id) + status
                });
            });
		...
		$scope.WizardClick = function (clickID) {
			if (clickID == 1) {
				...
				if((form.steps && $scope.tabId == form.steps) || (!form.steps && $scope.tabId == 3)){
					var data = GetOptsValues($scope, form, 'backups', fk_type, prev_opts);
