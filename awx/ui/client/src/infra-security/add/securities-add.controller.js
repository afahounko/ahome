        ...
        $scope.formSave = function() {
            ...
            Rest.post(data)
                .then(({data}) => {
                    ...
                    if (base === 'ipam_securities') {
                        $rootScope.flashMessage = i18n._('New Security successfully created!');
                        $rootScope.$broadcast("EditIndicatorChange", "Security", data.id);
                ...
                .catch(({data, status}) => {
                    ProcessErrors($scope, data, status, form, { hdr: i18n._('Error!'), msg: i18n._('Failed to add new Security. POST returned status: ') + status });
                });
        };

        $scope.formCancel = function() {
            $state.go('infraSecurityList');
        };
    }
];
