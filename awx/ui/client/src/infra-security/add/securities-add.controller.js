...
export default ['$window', '$scope', '$rootScope', '$stateParams', 'SecurityForm', 'GenerateForm', 'Rest','ParseTypeChange',
    ...
    function($window, $scope, $rootScope, $stateParams, SecurityForm, GenerateForm, Rest, ParseTypeChange, Alert,
	...
	$state, $location, i18n, ParseVariableString, $q, initSelect, cloudProcess, SetActiveWizard, GetOptsValues) {
		...
		$scope.WizardClick = function (clickID) {
			if (clickID == 1) {
				...
				if((form.steps && $scope.tabId == form.steps) || (!form.steps && $scope.tabId == 3)){
					var data = GetOptsValues($scope, form, 'securities', fk_type);
        ...
        // Save
        function deleteSecurity(id, _callback){
        	DeleteSubJobTemplate(defaultUrl, id, '', 0);
        	_callback();
        }
        $scope.formSave = function() {
        	var data_item = processNewData(form.fields);
        	SaveInfraItem(defaultUrl, form, data_item, 'infraSecurityList');
        };
