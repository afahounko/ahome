        ...
        return {
            ...
            */
            generateTree: function (params) {
                let form, list, formStates, listState, formss, boxes, itm,  
                    states = [];
				console.log("Generate Tree Begin");
                ...
                return new Promise((resolve) => {
                    ...
                    if (params.formss) {
                        // handle inconsistent typing of form definitions
                        // can be either an object or fn
                        formss = $injector.get(params.formss);
                    	console.log("Params.formss");
                    	console.log(formss);
                        formss = typeof (form) === 'function' ? formss() : formss;
                    	formStates = _.map(params.modes, (mode) => this.generateFormssNode(mode, formss, params));
                    	states = states.concat(_.flatten(formStates));
                	}
                	console.log(states);
            ...
            generateListNode: function (list, params, modaldlg) {
				...
				console.log("Generate List Node");
                ...
                function generateTemplateBlock() {
                    if (params.templates && params.templates.list) {
                        ...
                        return function (ListDefinition, generateList) {
                            ...
                            console.log('1');
                            if(params.box){
                                let html_box = generateList.buildBox($injector.get(params.box));
                                //html = generateList.wrapPanel(html_box) + html;
                                html = generateList.boxPanel(html_box) + html;
                            }
							console.log('2');
			...
			/**
             * @ngdoc method
             * @name stateDefinitions.generateListBoxesNode
             * @description builds single list node
             * @params {object} list - list definition/configuration object
             * @params {object} params
             * @returns {object} a list state definition
            ...
            generateListBoxesNode: function (list, params, boxes, modaldlg) {
				console.log("Generate Lists + Boxes Node");
				let state, url, name, category, box,
					states = [];
				// allows passed-in params to specify a custom templateUrl
                // otherwise, use html returned by generateList.build() to fulfill templateProvider fn
                console.log(boxes);
                for(category in boxes)
	            {
	            	for(box in boxes[category])
	            	{
	                	url = params.urls && params.urls.list ? params.urls.list : (params.url ? params.url : `/${list.name}/${category}/${box}`);
		                function generateTemplateBlock() {
		                    if (params.templates && params.templates.list) {
		                        return params.templates.list;
		                    } else {
		                        return function (ListDefinition, generateList) {
	                                ...
	                                let html_box = generateList.buildBox($injector.get(boxes[category][box]));
	                                console.log('2');
	                                html = generateList.wrapPanel(html_box) + html;
		                ...
		                state = $stateExtender.buildDefinition({
		                    searchPrefix: list.iterator,
		                    name: `${params.parent}.${category}.${box}`,
		                    url: url,
		                    data: params.data,
		                    ncyBreadcrumb: {
		                        label: list.title
            ...
            */
			...
			/**
             * @ngdoc method
             * @name stateDefinitions.generateFormssNode
             * @description builds a node of form states, e.g. resource.edit.** or resource.add.**
             * @param {string} mode - 'add' || 'edit' - the Formss mode of this node
             * @param {object} form - Formss definition/configuration object
             * @returns {array} Array of state definitions required by form mode [{...}, {...}, ...]
             */
            generateFormssNode: function (mode, formss, params) {
                let formNode, box, form, 
                    states = [],
                    url;
                var url_pref, name_pref;
                for(box in formss)
                {
                	form = formss[box];
                	$rootScope.id_type = form.name;
	                switch (mode) {
	                    case 'add':
	                    	url_pref = '';
	                    	name_pref = '';
	                    	if(form.boxid !== undefined){
	                    		url_pref = '/' + form.boxid;
	                    		name_pref = '.' + form.boxid;
	                    		if(form.subboxid !== undefined){
	                    			url_pref = url_pref + '/' + form.subboxid;
	                    			name_pref = name_pref + '.' + form.subboxid;
	                    		}
	                    	}
	                    	
	                        url = params.urls && params.urls.add ? params.urls.add : (params.url ? params.url : `/add_${form.name}`);
	                        // breadcrumbName necessary for resources that are more than one word like
	                        // job templates.  form.name can't have spaces in it or it busts form gen
	                        formNode = $stateExtender.buildDefinition({
	                            name: params.name || `${params.parent}.add_${form.name}`,
	                            url: url, 
	                            ncyBreadcrumb: {
	                                [params.parent ? 'parent' : null]: `${params.parent}`,
	                                label: i18n.sprintf(i18n._("CREATE %s"), i18n._(`${form.breadcrumbName || form.name.toUpperCase()}`))
	                            ...
	                            resolve: {
	                                'FormDefinition': [params.formss, function (definition) {
	                                    return definition;
	                                }]
	                            }
	                        });
	                    ...
	                    case 'edit':
	                    	...
	                    	url_pref = '';
	                    	name_pref = '';
	                    	console.log(name_pref);
	                        url = params.urls && params.urls.edit ? params.urls.edit : (params.url ? params.url : `${url_pref}/:${params.data.activityStreamTarget}_id`);
	                        let breadcrumbLabel = params.breadcrumbs && params.breadcrumbs.edit ? params.breadcrumbs.edit : '{{parentObject.name || name}}';
	                        let formNodeState = {
	                            name: params.name || `${params.parent}${name_pref}.edit_${form.name}`,
	                            url: url,
	                            ncyBreadcrumb: {
	                                [params.parent ? 'parent' : null]: `${params.parent}`,
	                                label: breadcrumbLabel
	                            ...
	                            resolve: {
	                                ...
	                                resourceData: ['FormDefinition', 'Rest', '$stateParams', 'GetBasePath',
	                                    function (FormDefinition, Rest, $stateParams, GetBasePath) {
	                                    	console.log('boxform1');
	                                        let form, path;
	                                        form = typeof (FormDefinition) === 'function' ?
	                                            FormDefinition() : FormDefinition;
	                                        console.log('boxform2');
	                                        if (GetBasePath(form[box].basePath) === undefined && GetBasePath(form[box].stateTree) === undefined) {
	                                            throw { name: 'NotImplementedError', message: `${form.name} form definition is missing basePath or stateTree property.` };
	                                        }
	                                        else {
	                                            path = (GetBasePath(form[box].basePath) || GetBasePath(form[box].stateTree) || form[box].basePath) + $stateParams[`${params.data.activityStreamTarget}_id`];
	                                        }
	                                        console.log('boxform3');
	                                        Rest.setUrl(path);
	                                        return Rest.get();
	                                    }
	                                ]
	                        ...
	                        if (params.data && params.data.activityStreamTarget) {
	                        	//console.log('boxform4');
	                            formNodeState.data = {};
	                            formNodeState.data.activityStreamId = params.data.activityStreamId ? params.data.activityStreamId : params.data.activityStreamTarget + '_id';
	                            formNodeState.data.activityStreamTarget = params.data.activityStreamTarget;
	                        }
	                        formNode = $stateExtender.buildDefinition(formNodeState);
							//console.log('boxform5');
	                        if (params.resolve && params.resolve.edit) {
	                            formNode.resolve = _.merge(formNode.resolve, params.resolve.edit);
	                        }
	                        break;
            ...
            generateFormListDefinitions: function (form, formStateDefinition, params) {
                ...
                function buildListDefinition(field) {
                	console.log("---Build List Definition---");
                    let state,
                        list = field.include ? $injector.get(field.include) : field,
