...
export default ['$window', '$scope', '$rootScope', 'DocumentationForm', 'GenerateForm', 'Rest','ParseTypeChange',
    'Alert', 'ProcessErrors', 'ReturnToCaller', 'GetBasePath',
    'Wait', 'CreateSelect2', '$state', '$location', 'i18n','ParseVariableString',
    function($window, $scope, $rootScope, DocumentationForm, GenerateForm, Rest, ParseTypeChange, Alert,
    ProcessErrors, ReturnToCaller, GetBasePath, Wait, CreateSelect2,
    $state, $location, i18n, ParseVariableString) {

        var defaultUrl = GetBasePath('ipam_documentations'),
        	id_type = $window.localStorage.getItem('form_id'),
            form = DocumentationForm[id_type];
        ...
        $scope.formSave = function() {
            ...
            Rest.post(data)
                .then(({data}) => {
                    ...
                    if (base === 'ipam_documentations') {
                        $rootScope.flashMessage = i18n._('New Documentation successfully created!');
                        $rootScope.$broadcast("EditIndicatorChange", "Documentation", data.id);
                        
                         $state.go('infraDocumentationsList', null, { reload: true });
                    } else {
                        ReturnToCaller(1);
                    }
                })
                .catch(({data, status}) => {
                    ProcessErrors($scope, data, status, form, { hdr: i18n._('Error!'), msg: i18n._('Failed to add new Documentation. POST returned status: ') + status });
                });
        };

        $scope.formCancel = function() {
            $state.go('infraDocumentationsList');
        };
    }
];
